...
def fancy_lane_detector(_input_image):
    """function to generate either Hough's lines only, or smooth lines"""
    # return generate_lines(_input_image)       # only Hough
...
def generate_lines(_input_copy):
    ...
    _output_image = weighted_img(_embedded_image, _input_copy, _alphaValue)
...
def generate_smooth_lanes(_input_copy):
    ...
    _hlines = hough_lines_only(_masked_edges, rho, theta, threshold, min_line_length, max_line_gap)
    ...
    _input_positive_yIntercept = np.mean(_input_positive_yIntercept[_input_positive_magnitude][-_numberOfLinesToConsider::])

    _input_negative_yIntercept = _input_negative_y_values - _input_negative_slopes * _input_negative_x_values
    _input_negative_yIntercept = np.mean(_input_negative_yIntercept[_input_negative_magnitude][-_numberOfLinesToConsider::])
    ...
    # right lane
    _output_positive_x1 = (_weighted_image.shape[0] - _input_positive_yIntercept) / _input_positive_slopes          # top
    _output_positive_y1 = _weighted_image.shape[0]
    _output_positive_x2 = (_weighted_image.shape[0] / 1.5 - _input_positive_yIntercept) / _input_positive_slopes    # bottom
    _output_positive_y2 = _weighted_image.shape[0] / 1.5
    # left lane
    _output_negative_x1 = (_weighted_image.shape[0] - _input_negative_yIntercept) / _input_negative_slopes          # top
    _output_negative_y1 = _weighted_image.shape[0]
    _output_negative_x2 = (_weighted_image.shape[0] / 1.5 - _input_negative_yIntercept) / _input_negative_slopes    # bottom
    ...
    # return _embedded_image      # returns the Hough's transformation with mask applied (Only 1 output instead of 9)
    # return _weighted_image      # returns Houghs mixed with original input. Not smooth
...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    line_img = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8)
    draw_lines(line_img, lines)
    return line_img      # modification to have access to both the lines and the merge
